(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))l(s);new MutationObserver(s=>{for(const t of s)if(t.type==="childList")for(const a of t.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&l(a)}).observe(document,{childList:!0,subtree:!0});function i(s){const t={};return s.integrity&&(t.integrity=s.integrity),s.referrerPolicy&&(t.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?t.credentials="include":s.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function l(s){if(s.ep)return;s.ep=!0;const t=i(s);fetch(s.href,t)}})();const h="modulepreload",w=function(e){return"/"+e},p={},b=function(o,i,l){let s=Promise.resolve();if(i&&i.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),r=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));s=Promise.allSettled(i.map(c=>{if(c=w(c),c in p)return;p[c]=!0;const n=c.endsWith(".css"),u=n?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${u}`))return;const d=document.createElement("link");if(d.rel=n?"stylesheet":h,n||(d.as="script"),d.crossOrigin="",d.href=c,r&&d.setAttribute("nonce",r),document.head.appendChild(d),n)return new Promise((_,v)=>{d.addEventListener("load",_),d.addEventListener("error",()=>v(new Error(`Unable to preload CSS for ${c}`)))})}))}function t(a){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=a,window.dispatchEvent(r),!r.defaultPrevented)throw a}return s.then(a=>{for(const r of a||[])r.status==="rejected"&&t(r.reason);return o().catch(t)})};function g(){const e=document.createElement("div");return e.innerHTML=`
    <div class="caja">
    ...
    `,e}g.afterRender=()=>{const e=document.getElementById("viewer3d");e&&b(()=>import("./model-viewer-ClFR6lzd.js"),[]).then(o=>{o.initModelViewer(e)})};function f(){const e=document.createElement("div");return e.innerHTML=`
      <!-- Spotify -->
...
<script src="scriptacordeon.js"><\/script>
    `,e}const x={"/":g,"/about":f,"/blog":y,"/proyectos":k};function m(){const e=location.hash.slice(1)||"/",o=document.getElementById("app"),i=x[e];if(i){o.innerHTML="";const l=i();o.appendChild(l),typeof i.afterRender=="function"&&i.afterRender()}else o.innerHTML="<h2>Página no encontrada</h2>"}window.addEventListener("hashchange",m);window.addEventListener("load",m);
